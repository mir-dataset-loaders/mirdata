...
.. admonition:: Dataset Info
    ...
    OpenMIC-2018 is a dataset of 20000 excerpts of polyphonic audio recordings in the
    Free Music Archive (FMA).
    This dataset was produced by researchers at Spotify and New York University.
    The collection has been partially annotated for the presence or absence of 20
    instrument categories by workers on a crowd-sourcing platform.

    Each excerpt is 10 seconds long, and no two excerpts come from the same
    recording.

    Each of the 20 instrument classes is guaranteed to have at least 500 positive
    examples and 1500 observations (positive or negative) in total.
    The style and genre of recordings is mixed and biased with respect to the FMA, 
    but designed to ensure sufficient representation of each instrument category.

    Note that the excerpts are partially annotated: only some of the instrument labels 
    will be observed for any track, and many are unobserved.
    ...
    In addition to raw audio, pre-computed features generated by the VGGish model are
    provided.
...
class Dataset(core.Dataset):
    ...
    def _metadata(self):
        ...
        metadata['track_genres'] = metadata['track_genres'].map(lambda x: json.loads(str_fixer.sub('\"', x)), na_action="ignore")
        ...
        for split in ['train', 'test']:
            split_df = pd.read_csv(Path(self.data_home) / Path('partitions') / f"split01_{split}.csv", header=None, index_col=0)
