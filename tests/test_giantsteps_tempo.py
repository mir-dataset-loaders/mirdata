...
def test_load_tempo():
    ...
    assert tempo_data == utils.TempoData(
        time=np.array([0.0]),
        duration=np.array([120.0]),
        value=np.array([137.6]),
    ...
    assert np.array_equal(tempo_data.time, np.array([0.0, 0.0]))
    assert np.array_equal(tempo_data.duration, np.array([120.0, 120.0]))
    assert np.array_equal(tempo_data.value, np.array([77.0, 139.0]))
