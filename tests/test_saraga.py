...
def test_track():
    ...
    expected_attributes = {
        ...
        'raaga': [
            {'uuid': '42dd0ccb-f92a-4622-ae5d-a3be571b4939', 'name': 'Śrīranjani'}
        ],
        'form': [{'name': 'Kriti'}],
    ...
    expected_property_types = {
        ...
        'tempo': dict,
        'phrases': classes.EventData,
        'pitch': classes.F0Data,
        'pitch_vocal': classes.F0Data,
        'sama': classes.SectionData,
...
def test_to_jams():
    ...
    assert metadata['taala'] == [
        {'uuid': 'c788c38a-b53a-48cb-b7bf-d11769260c4d', 'name': 'Ādi'}
    ]
    ...
    assert [section.time for section in sections_hindustani] == [3.298, 59.49, 679.834]
...
def test_load_pitch():
    ...
    # Check types
    assert type(parsed_pitch) == classes.F0Data
    ...
    # Check types
    assert type(parsed_vocal_pitch) == classes.F0Data
...
def test_load_sama():
    ...
    # Check types
    assert type(parsed_sama) == classes.SectionData
    assert type(parsed_sama.intervals) is np.ndarray
    assert type(parsed_sama.labels) is list

    # Check values
    assert np.array_equal(parsed_sama.intervals[:, 0], np.array([4.894, 10.229]))
    assert np.array_equal(parsed_sama.intervals[:, 1], np.array([10.229, 15.724]))
...
def test_load_sections():
    ...
    # Check types
    assert type(parsed_sections) == classes.SectionData
    ...
    assert np.array_equal(
        ...
        np.array([85.28979591699999, 166.857142856, 308.767346938]),
    )
    ...
    assert np.array_equal(
        parsed_sections.intervals[:, 1],
        np.array([58.236000000000004, 678.009, 894.433]),
    )
...
def test_load_phrases():
    ...
    # Check types
    assert type(parsed_phrases) is classes.EventData
    assert type(parsed_phrases.intervals) is np.ndarray
    assert type(parsed_phrases.event) is list

    # Check values
    assert np.array_equal(
        parsed_phrases.intervals[:, 0], np.array([0.224489795, 5.844897959, 8.50430839])
    )
    assert np.array_equal(
        parsed_phrases.intervals[:, 1],
        np.array([2.718367346, 8.318367346, 10.779863945]),
    )
...
def test_load_metadata():
    ...
    assert (
        parsed_metadata['data_home'] == 'tests/resources/mir_datasets/saraga/saraga1.0'
    )
