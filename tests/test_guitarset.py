...
def test_load_beats():
    assert np.allclose(TRACK.beats.beat_times, 
                       [0.50420168, 1.00840336, 1.51260504])
...
def test_load_contours():
    assert np.allclose(TRACK.pitch_contours['e'].times[:10],
                       [0.7670358269999724,
                        0.7728408159999844,
                        0.778645804000007,
                        0.7844507929999054,
                        ...
                        0.8134757359999867,
                        0.8192807249999987])
    assert np.allclose(TRACK.pitch_contours['e'].frequencies[:10],
                       [393.388,
                        393.301,
                        393.386,
                        393.348,
                        ...
                        393.352,
                        393.37])
    assert np.allclose(TRACK.pitch_contours['e'].confidence[:10],
                       np.ones((10,)))


def test_load_notes():
    assert np.allclose(TRACK.notes['e'].start_times,
                       [0.7612308390022235, 1.5072852607709137, 1.7806185941042258])
    assert np.allclose(TRACK.notes['e'].end_times,
                       [1.2604598639455844, 1.7336798185940552, 2.0])
    assert np.allclose(TRACK.notes['e'].notes,
                       [67.0576287044242, 71.03221526299762, 71.03297250121584])
    assert np.allclose(TRACK.notes['e'].confidence,
                       [1, 1, 1])
