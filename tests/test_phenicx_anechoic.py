...
def test_source():
    ...
    source = phenicx_anechoic.Source(
        name='bassoon1', stem_id=0, path=os.path.join(audio_home, 'bassoon1.wav')
    )
    y = source.audio
    sr = source.rate
...
def test_target():
    ...
    for i, instrument in enumerate(sources_names):
        sources.append(
            phenicx_anechoic.Source(
                name=instrument,
                stem_id=0,
                path=os.path.join(audio_home, instrument + '.wav'),
            )
        )
    target = phenicx_anechoic.Target(
        sources=sources, name='bassoon', instruments=['bassoon'], score_path=score_home
    )
    y = target.audio
    sr = target.rate
    assert sr == 44100, 'sample rate of PHENICX-Anechoic should be 44100'
...
def test_load_score():
    ...
    score_paths = [
        os.path.join(score_path, instrument + '.txt') for instrument in instruments
    ]
