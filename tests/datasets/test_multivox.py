...
def test_video_none_path(tmp_path):
    """Test that accessing video with no video_360_path raises FileNotFoundError."""
    meta = {"singer_ids": [], "locations_at_circle": 0}
    track = _make_track(tmp_path, meta, {})
    with pytest.raises(FileNotFoundError):
        _ = track.video
...
def test_video_calls_load_video(tmp_path):
    """Test that track.video property calls load_video with correct path."""
    ...
    # Create dummy video frames (5 frames, 240x320 RGB)
...
def test_load_video_basic():
    """Test basic video loading with moviepy."""
    # Create mock frames (5 frames, 480x640 RGB)
    mock_frames = [
    ...
    with patch("mirdata.datasets.multivox.VideoFileClip", return_value=mock_clip):
        result = multivox.load_video("fake_path.mp4")

        assert result.shape == (5, 480, 640, 3)
        assert np.array_equal(result, np.array(mock_frames))
