import os
from mirdata.datasets import fmakv2
from tests.test_utils import run_track_tests


def test_track():
    default_trackid = "10"
    data_home = os.path.normpath("tests/resources/mir_datasets/fmakv2")
    dataset = fmakv2.Dataset(data_home)
    track = dataset.track(default_trackid)

    expected_attributes = {
        "track_id": "10",
        "audio_path": os.path.join(
            os.path.normpath("tests/resources/mir_datasets/fmakv2/"),
            "000/000010.mp3",
        ),
        "key": "F#",
        "mode": "Major",
    ...
    assert sr == 11025
    assert audio.shape == (11025,)


def test_to_jams():
    data_home = "tests/resources/mir_datasets/fmakv2"
    default_trackid = "10"
    dataset = fmakv2.Dataset(data_home)
    track = dataset.track(default_trackid)
    jam = track.to_jams()
