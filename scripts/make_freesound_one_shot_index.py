...
ONE_SHOT_DATASET_PATH = "../mirdata/datasets/indexes/freesound_one_shot_percussive_sounds_index.json"
        ...
        if "one_shot_percussive_sounds" in subfolder:
            for subsubfolder in os.listdir(os.path.join(dataset_data_path, subfolder)):
                ...
                if '.' not in subsubfolder:
                    for track in os.listdir(os.path.join(dataset_data_path, subfolder, subsubfolder)):
                        dataset_index["tracks"][track.replace(".wav", "")] = {
                            "audio": [
                                os.path.join(subfolder, subsubfolder, track),
                                md5(os.path.join(dataset_data_path, subfolder, subsubfolder, track)),
                            ],
                            "analysis": [
                                os.path.join('analysis', subsubfolder, track.replace(".wav", "_analysis.json")),
                                md5(os.path.join(dataset_data_path, 'analysis', subsubfolder, track.replace(".wav", "_analysis.json"))),
                            ],
                        }
...
if __name__ == "__main__":
    PARSER = argparse.ArgumentParser(description="Make Freesound one-shot percussive sounds index file.")
    PARSER.add_argument(
        "dataset_data_path",
        type=str,
