...
def make_guitarset_index(guitarset_data_path):
    ...
    for track_id in track_ids:
        ...
        audio_hex_checksum = md5(
            os.path.join(audio_hex_dir, '{}_hex.wav'.format(track_id))
        )
        audio_mic_checksum = md5(
            os.path.join(audio_mic_dir, '{}_mic.wav'.format(track_id))
        )
        audio_mix_checksum = md5(
            os.path.join(audio_mix_dir, '{}_mix.wav'.format(track_id))
        )
